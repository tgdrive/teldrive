...
export const Route = createFileRoute("/share/$token")({ component: PublicSharePage });
...
function PublicSharePage() {
  ...
  const createFolder = async () => {
    ...
    try {
      await apiFetch(`/v1/public/shares/${encodeURIComponent(token)}/folders`, {
        ...
        body: JSON.stringify({ parentId: currentParentId, name, conflictPolicy: "fail" }),
      });
      setFolderName("");
      ...
      toast.error("Folder could not be created", { description: userMessage(cause) });
    }
  };
  ...
  const renameSelected = async () => {
    ...
    try {
      ...
      toast.error("Item could not be renamed", { description: userMessage(cause) });
    }
  };
  ...
  const trashSelected = async () => {
    ...
    try {
      ...
      toast.error("Items could not be moved to trash", { description: userMessage(cause) });
    }
  };
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background text-foreground">
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 px-4 py-5 sm:px-6">
        {loading && !share && !needsPassword ? (
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-3">
            ...
            <FileBrowser
              ...
              selectionOverlay={
                editable && selectedFiles.length ? (
                  <div className="pointer-events-none absolute inset-x-0 bottom-4 z-30 flex justify-center px-4">
                    <div className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-border bg-surface/95 p-1.5 shadow-xl backdrop-blur">
                      <span className="rounded-full bg-accent/10 px-3 py-2 text-sm font-medium text-accent">
