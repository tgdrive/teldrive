...
VALUES ($1,1001,'shared.bin','shared.bin','file','application/octet-stream',4,false,'active',now())`, fileID); err != nil {
	...
	if created.PublicURL.IsAbs() || !strings.HasPrefix(created.PublicURL.Path, "/v1/public/shares/") {
		t.Fatalf("public URL = %q, want relative share URL", created.PublicURL.String())
	}
